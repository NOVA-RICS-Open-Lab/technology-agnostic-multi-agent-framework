...
class EMA(Agent):
    ...
    class ASDS:
        ...
        def HandlePersistance(self):
            ...
            if getattr(self.agent, 'debugLevel', 0) >= 2 or self.debug:
                print("\n[DEBUG-L2][EMA] //////////////////////////// ECOSYSTEM OVERVIEW ////////////////////////////")
        ...
        def RegisterAgent(self, agentDescription: AgentDescription):
            ...
            if agentDescription.agentid.address and agentDescription.agentid.address.ip != self.agent.agentDescription.agentid.address.ip:
    ...
    def __init__(self, name=DEFAULT_EMA_NAME, debug: int = 1):
        super().__init__(name, debugLevel=debug)
        self.asds = self.ASDS(self, debug=debug!=0)
...
def LaunchEMA(debug: int = 1) -> Agent:
