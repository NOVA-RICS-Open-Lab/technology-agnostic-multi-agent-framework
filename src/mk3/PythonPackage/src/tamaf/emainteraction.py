...
class EMAInteraction:
    ...
    def _SendToEMA(self, aclMessage: ACLMessage) -> bool:
    ...
    def KeepAliveAgent(self):
        aclMessage = ACLMessage(
            ...
            receiver=[AgentID(DEFAULT_EMA_NAME, self.behavior.agent._mts.emaAddress)], 
    ...
    def RegisterAgent(self):
        ...
        if not self.messageSent:
            ...
            self._SendToEMA(registerACLMessage)
                
            self.messageSent = True

        if self.messageSent:
            responseMessage = self.behavior.Receive(ACLMessageTemplate(sender=AgentID(DEFAULT_EMA_NAME, self.behavior.agent._mts.emaAddress), performatives={Performative.INFORM}, ontology=DEFAULT_AGENTMANAGEMENT_ONTOLOGY_NAME, conversationID=self.conversationID))
    ...
    def DeRegisterAgent(self):
        ...
        if not self.messageSent:
            ...
            self._SendToEMA(deRegisterACLMessage)
    ...
    def ModifyAgent(self):
        ...
        if not self.messageSent:
            ...
            self._SendToEMA(modifyACLMessage)
    ...
    def Search(self, agentDescription: 'AgentDescription'):
        ...
        if not self.messageSent:
            ...
            self._SendToEMA(SearchACLMessage)
