...
class MTS:
    ...
    def Send(self, aclMessage: 'ACLMessage') -> bool:
        ...
        for receiver in aclMessage.receiver:
            try:
                ...
                self.agent.log_debug(1, f"Sent message to {receiver.GetFullID()} (Performative: {aclMessage.performative})")
                self.agent.log_debug(2, f"NEW MESSAGE SENT: {aclMessage.ToDict()}")

                transportmessage = TransportMessage(aclMessage, aclMessage.sender.address, receiverAddress)
                
                # Fire and forget async task
                loop = self.agent._ams.get_shared_loop()
    ...
    async def handle_client(self, reader, writer):
        ...
        if sock is not None:
            sock.setsockopt(socket.IPPROTO_TCP, socket.TCP_NODELAY, 1)
        self.agent.log_debug(2, f"Accepted connection from {peer}")
        try:
            ...
            self.agent.log_debug(2, f"NEW MESSAGE ARRIVED: {messageDict}")
            sender_id = aclMessage.sender.GetFullID() if (aclMessage.sender and hasattr(aclMessage.sender, 'GetFullID')) else "unknown"
            self.agent.log_debug(1, f"Received message from {sender_id} (Performative: {aclMessage.performative})")
    ...
    def StartTCPServer(self, address: Address):
        ...
        if self._stop_event is not None:
            import threading
            if threading.current_thread() is self.agent._ams._shared_loop_thread:
        ...
        self.agent.log_debug(1, f"Agent active at: {self.agent.agentDescription.agentid.address.GetString()}")
