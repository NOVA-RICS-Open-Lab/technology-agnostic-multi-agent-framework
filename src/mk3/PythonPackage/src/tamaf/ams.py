...
class AMS:
        ...
        def Action(self):
            if self.agent.agentDescription.agentid.name == DEFAULT_EMA_NAME:
                if not self.serverStarted:
                    self.agent._mts.StartTCPServer(Address(ALL_INTERFACES, self.agent._mts.emaAddress.port))
                    self.serverStarted = True
                if not self.agent._mts.serverOnline:
                    return
            ...
            if self.isRegistered is False:
                if self.agent._mts.serverOnline is False:
                    
                    if self.serverStarted:
                        return
                    
                    self.agent._mts.StartTCPServer(Address(ALL_INTERFACES, self.agent._mts.registeraddress.port))
                    self.serverStarted = True
                    if not self.agent._mts.serverOnline:
                        return
                ...
                if concept == DEFAULT_AGENTMANAGEMENT_ONTOLOGY_AGENTADDRESS_CONCEPT:
                    self.agent._mts.StopMessageServer()
                    newAddress = AgentManagementOntology.GetAgentAddressFromAttributes(agree.content)

                    self.agent._mts.serverOnline = False
                    self.agent._mts.StartTCPServer(Address(ALL_INTERFACES, newAddress.port))
            ...
            self.agent.Setup(self.emaInteraction)
            self.heartBeatBehavior = self.agent._ams.HeartBeatBehavior(self.agent)
    ...
    def __init__(self, agent: 'Agent'):
        ...
        self.activeBehaviorList: list['Behavior'] = []
        self.pendingAdditions: list['Behavior'] = []
        self.pendingRemovals: list['Behavior'] = []
        self.blockedBehaviorList: list['Behavior'] = []
            ...
            try:
                ...
                if currentLifeCycleState == LifeCycleState.SUSPENDED:
                    if self.heartBeatBehavior:
                        if self.agent._mts.newMessageEvent.is_set():
                            if self.heartBeatBehavior.IsBlocked():
                                self.heartBeatBehavior.UnBlock()
                            self.agent._mts.newMessageEvent.clear()
                    ...
                    for behavior in self.pendingRemovals:
                        if behavior in self.activeBehaviorList:
                            ...
                            self.pendingRemovals.remove(behavior)
                    ...
                    if closingBehavior.Done():
                        ...
                        self.agent._mts.StopMessageServer()
                        self.agent.log_debug(1, "Agent Taken Down")
                        break
    ...
    def BlockBehavior(self, behavior: 'Behavior'):
        if behavior in self.activeBehaviorList:
            self.activeBehaviorList.remove(behavior)
