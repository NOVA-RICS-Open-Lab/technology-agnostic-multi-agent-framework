...
std::optional<tamaf::messaging::ACLMessage> EMAInteraction::RegisterAgent() {
    if (!messageSent) {
        ...
        behavior->getAgent()->logDebug("EMAInteraction: Sending Register Message to EMA...");
        if (SendToEMA(registerACLMessage)) {
            behavior->getAgent()->logDebug("EMAInteraction: SendToEMA returned true.");
            messageSent = true;
        } else {
            behavior->getAgent()->logDebug("EMAInteraction: SendToEMA returned false.");
        }
    }
    ...
    if (messageSent) {
        ...
        tamaf::messaging::ACLMessage responseMessage = behavior->Receive(tmpl);
        if (AgentManagementOntology::Validate(responseMessage.getContent())) {
            messageSent = false;
...
std::optional<std::vector<tamaf::types::AgentDescription>> EMAInteraction::Search(const tamaf::types::AgentDescription& agentDescription) {
    ...
    if (messageSent) {
        ...
        if (AgentManagementOntology::Validate(responseMessage.getContent())) {
            messageSent = false;
            return AgentManagementOntology::GetAgentDescriptionsFromAttributes(responseMessage.getContent());
        }
    }
    return std::nullopt;
}
