...
public class MTS {
    ...
    // Dedicated thread pool for outgoing non-blocking I/O tasks if needed
                            ...
                            try {
                                ...
                                client.write(buf, null, new CompletionHandler<Integer, Void>() {
                                    ...
                                    public void completed(Integer result, Void attachment) {
                                        ...
                                        // Fire and forget, we don't strictly need to wait for the 200 OK
                                        // to return boolean since we're async now.
                                        try {
                                            client.close();
                                        } catch (IOException e) {}
                                    }
    ...
    private void processIncomingAsync(AsynchronousSocketChannel client) {
        ...
        client.read(sizeBuf, null, new CompletionHandler<Integer, Void>() {
            @Override
            public void completed(Integer result, Void attachment) {
                if (result < 4) {
                    try { client.close(); } catch (IOException e) {}
                ...
                client.read(dataBuf, null, new CompletionHandler<Integer, Void>() {
                    @Override
                    public void completed(Integer result, Void attachment) {
                        try {
                            dataBuf.flip();
                            ...
                            client.write(respBuf, null, new CompletionHandler<Integer, Void>() {
                                @Override
                                public void completed(Integer result, Void attachment) {
                                    try { client.close(); } catch (IOException e) {}
                                }
