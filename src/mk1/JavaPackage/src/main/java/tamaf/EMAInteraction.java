...
public class EMAInteraction {
    ...
    public void sendToEMA(ACLMessage msg) {
    ...
    public ACLMessage registerAgent() {
        ...
        if (messageSent) {
            ...
            System.out.println(behavior.getAgent().getAgentDescription().getAgentid().getFullID() + ": AGUARDANDO RESPOSTA DO EMA");
            ACLMessage response = behavior.receive(template);
            System.out.println(behavior.getAgent().getAgentDescription().getAgentid().getFullID() + ": RESPOSTA RECEBIDA DO EMA: " + (response != null ? response.getContent() : "null"));
            if (response != null) {
                messageSent = false;
                return response;
            }
